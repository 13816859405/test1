...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
            ...
            $.ajax({
            	...
            	success:function(result){
            		alert(result);
            		$("#span").after('<br/><input name="address" type="radio" id="address0" value="'+addr+'"/><span>'+addr+'</span>');
            		$("#addAddr").val("");
            	}
            });
        }else{
    ...
    $(".delete").find("a").click(function(){
       ...
       if(confirm("确定要删除吗？")) {
            ...
            if(tr) tr.remove();*/
            location.href="ProductServlet?pro=shoppingCart&opr=del&epId="+epId;
        }else{
            return false;
        }
    });
    ...
    $(".number").find("span").click(function(){
    	...
    	location.href="ProductServlet?pro=shoppingCart&opr=update&epId="+epId+"&num="+num;
    ...
    $("#logout").click(function(){
        if(confirm("是否确定注销?")) {
            location.href="UserLoginServlet?action=exite";
        }else{
    ...
    $("#userId").blur(function(){
		...
		$.ajax({
			type:"POST",
			url:"RegisterServlet?pro=findName",
			data:"userName="+userName,
			dataType:"json",
			success:function(ret){
				
				if(ret!=null){
					$("#userId").next().html("<font color='red'>用户已存在，不能使用</font>").attr("class","error");
				}
