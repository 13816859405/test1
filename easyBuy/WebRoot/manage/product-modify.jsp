...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改商品</h2>
		<div class="manage">
			<form action="ProductServlet?pro=upateProduct&epId=${product.epId }" method="post" enctype="multipart/form-data">
				<table class="form">
					<tr>
					...
					<tr>
						...
						<td>
							...
							<c:forEach items="${map }" var="pCate">
								<option value="${pCate.key.parentId }">${pCate.key.epcName }</option>
								<c:forEach items="${pCate.value }" var="cCate">
									<option value="${cCate.epcId}" 
									<c:if test="${cCate.epcId==product.epcId }">selected='selected'</c:if>>
									├ ${cCate.epcName}</option>
								</c:forEach>
							</c:forEach>
					...
					<tr>
						...
						<td><input type="file" class="text tiny" name="photo"/>${product.fileName }</td>
					</tr>
					<tr>
						<td></td>
