...
$(function(){
	$(".viewBill").on("click",function(){
		...
		$.ajax({
			url:path+"/bill/showbill/"+obj.attr("billid"),
			type:"get",
			dataType:"json",
			success:function(ret){
				$("#billCode").val(ret.billCode);
				$("#productName").val(ret.productName);
				$("#productUnit").val(ret.productUnit);
				$("#productCount").val(ret.productCount);
				$("#totalPrice").val(ret.totalPrice);
				$("#providerName").val(ret.proName);
				$("#isPayment").val(ret.isPayment);
				if(ret.isPayment=="1"){
					$("#isPayment").val("未付款");
				}else if(ret.isPayment=="2"){
					$("#isPayment").val("已付款");
				}
			},
			error:function(ret){
