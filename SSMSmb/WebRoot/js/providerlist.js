...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/pro/del/"+obj.attr("proid"),
		...
		success:function(data){
			if(data.ret == "true"){//删除成功：移除删除行
				...
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.ret+"】条订单，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		$.ajax({
			url:path+"/pro/showpro/"+obj.attr("proid"),
			type:"get",
			dataType:"json",
			success:function(ret){
				$("#proCode").val(ret.proCode);
				$("#proName").val(ret.name);
				$("#proContact").val(ret.proContact);
				$("#proPhone").val(ret.proPhone);
				$("#proFax").val(ret.proFax);
				$("#proDesc").val(ret.desc);
			},
			error:function(ret){
				alert("error!");
			}
